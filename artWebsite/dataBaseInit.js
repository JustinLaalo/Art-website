...
for(let i = 0; i < art.length; i++){
	artUsers.push({name: art[i].artist,followings: [], reveiwed:[],roles: "Artist", WorkShop:[],likes:[]});
	art[i].review = [];
	art[i].likes = [];
}
MongoClient.connect("mongodb://127.0.0.1:27017/", { useNewUrlParser: true }, function(err, client) {
  ...
  db.dropCollection("art", function(err, result){
	  if(err){
		...
		db.collection("art").insertMany(art, function(err, result){
			if(err) throw err;
			console.log("Successfuly inserted " + result.insertedCount + " art.")
			process.exit();
		})
  });
  db.dropCollection("user", function(err, result){
	...
	db.collection("user").insertMany(artUsers, function(err, result){
		if(err){
			console.log("Error dropping collection. Likely case: collection did not exist (don't worry unless you get other errors...)")
		}else{
	...
	// 	let info = {username:"Justin", password: "123"};
